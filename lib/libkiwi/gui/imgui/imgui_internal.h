...
#define ImAtof(STR)         ksl::atof(STR)
...
inline float  ImLog(float x)             { return kiwi::Log(x); }             // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
inline double ImLog(double x)            { return kiwi::Log(x); }
inline int    ImAbs(int x)               { return x < 0 ? -x : x; }
inline float  ImAbs(float x)             { return kiwi::Abs(x); }
inline double ImAbs(double x)            { return kiwi::Abs(x); }
